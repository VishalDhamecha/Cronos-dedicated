...
import com.company.mysapcpsdkprojectoffline.archcomp.SingleLiveEvent
import com.company.mysapcpsdkprojectoffline.repository.OperationResult.Operation
 ...
 * @param aPI_SALES_ORDER_SRV_Entities OData service
...
class Repository<T : EntityValue>(
        private val aPI_SALES_ORDER_SRV_Entities: API_SALES_ORDER_SRV_Entities,
    ...
    fun read() {
        ...
        aPI_SALES_ORDER_SRV_Entities.executeQueryAsync(dataQuery,
            { result ->
    ...
    fun read(parent: EntityValue, navigationPropertyName: String) {
        ...
        aPI_SALES_ORDER_SRV_Entities.loadPropertyAsync(navigationProperty, parent, dataQuery,
            {
    ...
    fun create(newEntity: T, media: StreamBase) {
        if (newEntity.entityType.isMedia) {
            aPI_SALES_ORDER_SRV_Entities.createMediaAsync(newEntity, media,
                {
    ...
    fun create(newEntity: T) {
        ...
        aPI_SALES_ORDER_SRV_Entities.createEntityAsync(newEntity,
            {
                insertToCache(newEntity, entities)
                if (relatedEntities.size > 0) {
                    insertToCache(newEntity, relatedEntities)
    ...
    fun update(updateEntity: T) {
        aPI_SALES_ORDER_SRV_Entities.updateEntityAsync(updateEntity,
            {
    ...
    fun delete(deleteEntities: MutableList<T>) {
        ...
        aPI_SALES_ORDER_SRV_Entities.applyChangesAsync(deleteChangeSet,
            {
    ...
    fun downloadMedia(entity:T, successHandler:Action1<ByteArray>,
        failureHandler:Action1<RuntimeException>) {
            aPI_SALES_ORDER_SRV_Entities.downloadMediaAsync(entity,
