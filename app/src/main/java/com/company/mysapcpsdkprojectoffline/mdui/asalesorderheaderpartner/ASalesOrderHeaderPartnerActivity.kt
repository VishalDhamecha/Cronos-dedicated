 ...
 * - [ASalesOrderHeaderPartnerCreateFragment]: to create or edit a(n) ASalesOrderHeaderPartnerType,
 * - [ASalesOrderHeaderPartnerDetailFragment]: to display the details of a(n) ASalesOrderHeaderPartnerType,
 * - [ASalesOrderHeaderPartnerListFragment]: to list ASalesOrderHeaderPartner.
 *
 ...
 * what to do.
 */
class ASalesOrderHeaderPartnerActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<ASalesOrderHeaderPartnerType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ASalesOrderHeaderPartnerListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: ASalesOrderHeaderPartnerType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link ASalesOrderHeaderPartnerCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: ASalesOrderHeaderPartnerType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ASalesOrderHeaderPartnerDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, ASalesOrderHeaderPartnerDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = ASalesOrderHeaderPartnerCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: ASalesOrderHeaderPartnerType?) {
        val updateFragment = ASalesOrderHeaderPartnerCreateFragment()
        val arguments = Bundle()
        ...
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            if (activity != null) {
                ...
                builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                    (fragmentActivity as ASalesOrderHeaderPartnerActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
                }
    ...
    internal class DelelteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            if (activity !=null) {
                val fragmentActivity = activity as AppCompatActivity
                val builder = AlertDialog.Builder(ContextThemeWrapper(fragmentActivity, R.style.AlertDialogStyle))
                val viewModel = ViewModelProvider(fragmentActivity).get(ASalesOrderHeaderPartnerTypeViewModel::class.java)
