 ...
 * - [ASalesOrderItemCreateFragment]: to create or edit a(n) ASalesOrderItemType,
 * - [ASalesOrderItemDetailFragment]: to display the details of a(n) ASalesOrderItemType,
 * - [ASalesOrderItemListFragment]: to list ASalesOrderItem.
 *
 ...
 * what to do.
 */
class ASalesOrderItemActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<ASalesOrderItemType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ASalesOrderItemListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: ASalesOrderItemType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link ASalesOrderItemCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: ASalesOrderItemType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ASalesOrderItemDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, ASalesOrderItemDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = ASalesOrderItemCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: ASalesOrderItemType?) {
        val updateFragment = ASalesOrderItemCreateFragment()
        val arguments = Bundle()
        ...
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            if (activity != null) {
                ...
                builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                    (fragmentActivity as ASalesOrderItemActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
                }
    ...
    internal class DelelteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            if (activity !=null) {
                val fragmentActivity = activity as AppCompatActivity
                val builder = AlertDialog.Builder(ContextThemeWrapper(fragmentActivity, R.style.AlertDialogStyle))
                val viewModel = ViewModelProvider(fragmentActivity).get(ASalesOrderItemTypeViewModel::class.java)
