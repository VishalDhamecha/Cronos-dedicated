 ...
 * - [ASalesOrderTextCreateFragment]: to create or edit a(n) ASalesOrderTextType,
 * - [ASalesOrderTextDetailFragment]: to display the details of a(n) ASalesOrderTextType,
 * - [ASalesOrderTextListFragment]: to list ASalesOrderText.
 *
 ...
 * what to do.
 */
class ASalesOrderTextActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<ASalesOrderTextType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ASalesOrderTextListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: ASalesOrderTextType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link ASalesOrderTextCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: ASalesOrderTextType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ASalesOrderTextDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, ASalesOrderTextDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = ASalesOrderTextCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: ASalesOrderTextType?) {
        val updateFragment = ASalesOrderTextCreateFragment()
        val arguments = Bundle()
        ...
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            if (activity != null) {
                ...
                builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                    (fragmentActivity as ASalesOrderTextActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
                }
    ...
    internal class DelelteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            if (activity !=null) {
                val fragmentActivity = activity as AppCompatActivity
                val builder = AlertDialog.Builder(ContextThemeWrapper(fragmentActivity, R.style.AlertDialogStyle))
                val viewModel = ViewModelProvider(fragmentActivity).get(ASalesOrderTextTypeViewModel::class.java)
