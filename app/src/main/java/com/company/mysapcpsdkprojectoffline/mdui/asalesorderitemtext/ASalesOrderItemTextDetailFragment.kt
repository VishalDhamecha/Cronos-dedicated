...
import com.company.mysapcpsdkprojectoffline.databinding.FragmentAsalesorderitemtextDetailBinding
...
/**
 * A fragment representing a single ASalesOrderItemTextType detail screen.
 * This fragment is contained in an ASalesOrderItemTextActivity.
 */
class ASalesOrderItemTextDetailFragment : InterfacedFragment<ASalesOrderItemTextType>() {

    /** Generated data binding class based on layout file */
    private lateinit var binding: FragmentAsalesorderitemtextDetailBinding

    /** ASalesOrderItemTextType entity to be displayed */
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        activity?.let {
            ...
            viewModel.selectedEntity.observe(viewLifecycleOwner, Observer { entity ->
                aSalesOrderItemTextTypeEntity = entity
                binding.setASalesOrderItemTextType(entity)
                setupObjectHeader()
            })
        }
    }
    ...
    private fun onDeleteComplete(result: OperationResult<ASalesOrderItemTextType>) {
        ...
        listener?.onFragmentStateChange(UIConstants.EVENT_DELETION_COMPLETED, aSalesOrderItemTextTypeEntity)
    }
    ...
    fun onNavigationClickedToASalesOrder_to_SalesOrder(view: View) {
        ...
        intent.putExtra("parent", aSalesOrderItemTextTypeEntity)
    ...
    private fun setupDataBinding(inflater: LayoutInflater, container: ViewGroup?): View {
        binding = FragmentAsalesorderitemtextDetailBinding.inflate(inflater, container, false)
     ...
     */
    private fun setDetailImage(objectHeader: ObjectHeader, aSalesOrderItemTextTypeEntity: ASalesOrderItemTextType) {
        if (aSalesOrderItemTextTypeEntity.getDataValue(ASalesOrderItemTextType.salesOrder) != null && aSalesOrderItemTextTypeEntity.getDataValue(ASalesOrderItemTextType.salesOrder).toString().isNotEmpty()) {
            objectHeader.detailImageCharacter = aSalesOrderItemTextTypeEntity.getDataValue(ASalesOrderItemTextType.salesOrder).toString().substring(0, 1)
        } else {
    ...
    /**
     * Setup ObjectHeader with an instance of aSalesOrderItemTextTypeEntity
     */
    private fun setupObjectHeader() {
        val secondToolbar = currentActivity.findViewById<Toolbar>(R.id.secondaryToolbar)
        if (secondToolbar != null) {
            secondToolbar.setTitle(aSalesOrderItemTextTypeEntity.entityType.localName)
        } else {
            currentActivity.setTitle(aSalesOrderItemTextTypeEntity.entityType.localName)
        }
        ...
        objectHeader?.let {
            it.apply {
                ...
                subheadline = EntityKeyUtil.getOptionalEntityKey(aSalesOrderItemTextTypeEntity)
            ...
            setDetailImage(it, aSalesOrderItemTextTypeEntity)
