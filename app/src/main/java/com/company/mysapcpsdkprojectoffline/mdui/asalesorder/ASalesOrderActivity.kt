 ...
 * - [ASalesOrderCreateFragment]: to create or edit a(n) ASalesOrderType,
 * - [ASalesOrderDetailFragment]: to display the details of a(n) ASalesOrderType,
 * - [ASalesOrderListFragment]: to list ASalesOrder.
 *
 ...
 * what to do.
 */
class ASalesOrderActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<ASalesOrderType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ASalesOrderListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: ASalesOrderType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link ASalesOrderCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: ASalesOrderType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ASalesOrderDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, ASalesOrderDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = ASalesOrderCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: ASalesOrderType?) {
        val updateFragment = ASalesOrderCreateFragment()
        val arguments = Bundle()
        ...
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            if (activity != null) {
                ...
                builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                    (fragmentActivity as ASalesOrderActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
                }
    ...
    internal class DelelteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            if (activity !=null) {
                val fragmentActivity = activity as AppCompatActivity
                val builder = AlertDialog.Builder(ContextThemeWrapper(fragmentActivity, R.style.AlertDialogStyle))
                val viewModel = ViewModelProvider(fragmentActivity).get(ASalesOrderTypeViewModel::class.java)
