 ...
 * - [ASalesOrderItemTextCreateFragment]: to create or edit a(n) ASalesOrderItemTextType,
 * - [ASalesOrderItemTextDetailFragment]: to display the details of a(n) ASalesOrderItemTextType,
 * - [ASalesOrderItemTextListFragment]: to list ASalesOrderItemText.
 *
 ...
 * what to do.
 */
class ASalesOrderItemTextActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<ASalesOrderItemTextType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ASalesOrderItemTextListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: ASalesOrderItemTextType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link ASalesOrderItemTextCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: ASalesOrderItemTextType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ASalesOrderItemTextDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, ASalesOrderItemTextDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = ASalesOrderItemTextCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: ASalesOrderItemTextType?) {
        val updateFragment = ASalesOrderItemTextCreateFragment()
        val arguments = Bundle()
        ...
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            if (activity != null) {
                ...
                builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                    (fragmentActivity as ASalesOrderItemTextActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
                }
    ...
    internal class DelelteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            if (activity !=null) {
                val fragmentActivity = activity as AppCompatActivity
                val builder = AlertDialog.Builder(ContextThemeWrapper(fragmentActivity, R.style.AlertDialogStyle))
                val viewModel = ViewModelProvider(fragmentActivity).get(ASalesOrderItemTextTypeViewModel::class.java)
