 ...
 * - [ASalesOrderHeaderPrElementCreateFragment]: to create or edit a(n) ASalesOrderHeaderPrElementType,
 * - [ASalesOrderHeaderPrElementDetailFragment]: to display the details of a(n) ASalesOrderHeaderPrElementType,
 * - [ASalesOrderHeaderPrElementListFragment]: to list ASalesOrderHeaderPrElement.
 *
 ...
 * what to do.
 */
class ASalesOrderHeaderPrElementActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<ASalesOrderHeaderPrElementType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ASalesOrderHeaderPrElementListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: ASalesOrderHeaderPrElementType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link ASalesOrderHeaderPrElementCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: ASalesOrderHeaderPrElementType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ASalesOrderHeaderPrElementDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, ASalesOrderHeaderPrElementDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = ASalesOrderHeaderPrElementCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: ASalesOrderHeaderPrElementType?) {
        val updateFragment = ASalesOrderHeaderPrElementCreateFragment()
        val arguments = Bundle()
        ...
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            if (activity != null) {
                ...
                builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                    (fragmentActivity as ASalesOrderHeaderPrElementActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
                }
    ...
    internal class DelelteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            if (activity !=null) {
                val fragmentActivity = activity as AppCompatActivity
                val builder = AlertDialog.Builder(ContextThemeWrapper(fragmentActivity, R.style.AlertDialogStyle))
                val viewModel = ViewModelProvider(fragmentActivity).get(ASalesOrderHeaderPrElementTypeViewModel::class.java)
